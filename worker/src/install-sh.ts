// install.sh renderer — v0.1.1 tarball-only, zero-GUI.
//
// Philosophy: Maxbridge is a background daemon. No .app. No Applications
// folder. No macOS TCC prompts. Pure Node process managed by launchd.
//
// Flow (linear, no branches, no polling):
//   1. Pre-flight (macOS 13+, Apple Silicon)
//   2. Aggressive clean: kill any prior daemon; REMOVE any legacy
//      /Applications/Maxbridge.app or ~/Applications/Maxbridge.app (they
//      are the source of TCC "would like to access data from other apps"
//      prompts and of the old "What should Maxbridge connect?" wizard).
//   3. Install Homebrew if missing
//   4. Install Claude CLI via Homebrew if missing
//   5. `claude setup-token` — Anthropic browser OAuth (the ONLY manual step)
//   6. Download the daemon TARBALL (not DMG) + SHA256 verify; extract
//      server-bundle + node-runtime into ~/.maxbridge/
//   7. Write launchd plist and bootstrap — persistent background daemon on
//      127.0.0.1:7423, auto-restarts on crash
//   8. Patch ~/.openclaw/openclaw.json (timestamped backup); route main
//      agent to maxbridge/claude-opus-4-7; kickstart openclaw gateway
//   9. End-to-end test: real Opus 4.7 round-trip → REPORT_STATUS=success

export function renderInstallSh(args: {
  licenseJwt: string;
  dmgUrl: string;    // Kept for backwards-compat in the render args; unused now.
  dmgSha256: string; // Same.
  licenseApiBase: string;
  landingUrl: string;
  version: string;
}): string {
  // Hard-coded tarball URL — pinned to a GitHub Release asset so every
  // install gets the exact same artifact. No DNS lag, CDN-backed.
  const TARBALL_URL = 'https://github.com/mbmarsirius/maxbridge/releases/download/v0.1.0/maxbridge-daemon-v0.1.1-darwin-arm64.tar.gz';
  const TARBALL_SHA256 = '1df890ae9dd7159c2c2b33b820ecc7ca882277f0075c121c4685863ae2de6570';

  return `#!/usr/bin/env bash
# Maxbridge installer — v${args.version} (tarball-only, zero-GUI)
...
export MAXBRIDGE_TARBALL_URL=${'"'}${TARBALL_URL}${'"'}
export MAXBRIDGE_TARBALL_SHA256=${'"'}${TARBALL_SHA256}${'"'}
